(function () {
  const GAME_SLUG = "missile-strike";
  const LOGIN_URL = "/login";
  const LEADERBOARD_URL = "/leaderboards/missile-strike";
  ...
  console.info("[MissileStrikeBridge] score-bridge.js loaded");
  ...
  async function submitScore(payload) {
    console.info("[MissileStrikeBridge] reportGameOver called", payload);

    if (!payload || typeof payload !== "object") {
      console.warn("[MissileStrikeBridge] Ignoring invalid game-over payload.");
      return;
    }
    ...
    if (!Number.isFinite(score) || score < 0) {
      console.warn("[MissileStrikeBridge] Ignoring invalid score.", payload.score);
    ...
    if (fingerprint === lastSubmissionFingerprint) {
      console.info("[MissileStrikeBridge] Duplicate game-over payload ignored.");
      return;
    }

    lastSubmissionFingerprint = fingerprint;

    // TODO: Harden score validation and anti-cheat checks before public competitive launch.
    ...
    if (!response) {
      console.error("[MissileStrikeBridge] Score submission failed before a response was received.");
      renderStatus("Couldn't save score. You can keep playing.");
      return;
    }

    console.info("[MissileStrikeBridge] /api/scores returned status", response.status);
    const result = await response.json().catch(() => null);

    if (response.status === 401 || result?.code === "login_required") {
      console.warn("[MissileStrikeBridge] Score submission requires login.");
    ...
    if (!response.ok || !result?.ok) {
      console.error("[MissileStrikeBridge] Score submission failed.", result);
      renderStatus("Couldn't save score. You can keep playing.");
      return;
    }

    console.info("[MissileStrikeBridge] Score submission succeeded.", result);
  ...
  window.MrDevGuyManMissileStrike = {
    reportGameOver: submitScore,
    resetPendingSubmission: function () {
      console.info("[MissileStrikeBridge] Submission fingerprint reset.");
